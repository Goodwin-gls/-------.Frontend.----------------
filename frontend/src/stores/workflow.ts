...
export const useWorkflowStore = defineStore('workflow', () => {
  ...
  async function fetchSteps() {
    loading.value = true
    try {
      steps.value = await workflowApi.getWorkflow(workflowName.value)
    } catch (error) {
  ...
  async function updateStepCoordinates(initialIndex: number, x: number, y: number) {
    try {
  ...
  async function updateStepName(initialIndex: number, name: string) {
    // Check uniqueness
    const exists = steps.value.some(s => s.name === name && s.initialIndex !== initialIndex)
    if (exists) {
